...
def _terminal_width() -> int:
    """获取终端真实列宽（2026-08-20：分隔线没到底——shutil 拿不到宽度时回退 80 导致线断在中途）。
...
def full_rule(char="─", color=C.LIGHT_BLUE, extra=0):
    """贯穿整个终端宽度的分隔线（2026-08-20：分割线要一直到底/铺满全宽）。

    用**半角字符**（默认 ─）：1 字符恒占 1 列，直接铺满「终端列数 + extra」个——
    任何终端都绝对到头（extra 默认 0：从 +30 → +10 → 0 逐轮微调，最终定格铺满）。
